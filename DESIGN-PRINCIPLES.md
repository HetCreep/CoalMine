...
| 4 | **Minimum necessary power** | Tokens are spent only when and where needed: lean always-loaded surfaces (descriptions, shared block), progressive disclosure (`references/` load only during a scan), tiers that scale cost to scope. **Spending the user's tokens always requires consent** — per-instance (a question menu) or standing (installed auto-cadence, capped at QUICK/report-only, with documented opt-outs). A silent expensive operation is theft, not a feature; updates and re-grounding are always offered, never auto-run. |
...
Relationship to other rule layers: the [Phoenix 13 Commandments](docs/hooks-safety.md) implement principles 6–8 and 10 for hooks; [scripts-quality](docs/scripts-quality.md) implements 2 and 6 for CLI scripts; this file governs everything. (The same two documents are installed into each machine's local rules home, where the gold-standard lifecycle re-validates their stamps.)
