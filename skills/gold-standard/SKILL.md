...
4. **CONFORM** (retrofit existing code) — adoption binds *future* work only; the existing codebase may still violate the new rules. After ADOPT, **offer this as a selectable choice** (use the host's choice UI — e.g. AskUserQuestion — not a free-text question):
   - **Conform now** — scan the existing project against the adopted ruleset, report each violation (`path:line` · rule · evidence), then fix **on approval**, each through the fix-mode safety harness: **green baseline first** (already-red → report-only, name the failure) → **checkpoint** (no git / no restore point, or **non-interactive / CI** → report-only, never auto-edit; never silently stash the user's WIP) → **one fix → re-run build + covering tests** → **verify-loop** (newly-red vs baseline ⇒ auto-revert only files this fix wrote + downgrade to report-only · green ⇒ keep) → **batch re-run** of the suite if >1 fix → **diff summary**. If any harness step fails → stop, restore the checkpoint, report. ⚠️ A rule-conformance fix that **no test covers** (most don't — adding a disclaimer, masking a username, routing through a redactor) can't be verify-loop-checked — apply only if structurally trivial; otherwise leave it to explicit pick with an "untested path" warning. Green tests that never exercise the changed behavior aren't verification.
